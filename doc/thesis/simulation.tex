...
The assumption is that none, for the simulation relevant, events are happening in between two consecutive events.
...
Such a erroneous behavior must be corrected by the simulation core by speeding up the simulation, for example by decreasing idle times between events.
...
To validate an real-time simulation regarding timing quality the performance ration can be used.
...
%TODO: find connection to next chapter