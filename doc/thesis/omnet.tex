...
Configurations like the path to load the \emph{NED} files can be defined within a configuration files as well as by a command line option (\emph{-n}).
With multiple configurations the values are merged into a resulting configuration.

The selection of the user interface can be defined fia the command line option \emph{-u}.
...
With \emph{oop\_run} or any other simulation executable other available command line options are available via \emph{-h}. \cite[section 10.1.1, section 10.1.2]{omnet_manual}

\subsection{Configuration}
\label{sec:omnet_running_config}
The configuration file can be given by a command line parameter, by default the \emph{omnetpp.ini} file is used.
...
%TODO reference and explanation of tcl/tkl
